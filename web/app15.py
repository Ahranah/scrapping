...
# 크롤링 실행 함수
def run_crawler(username, password, search_key):
    crawler_file = subprocess.run([sys.executable, "financial_page2.py", "run_selenium", username, password, search_key], capture_output=True, text=True)
    file_name = crawler_file.stdout.strip()  # financial_page2.py가 출력한 파일명
    return file_name
...
def index():
    if request.method == "POST":
        ...
        file_name = run_crawler(username, password, search_key)
        
        if not file_name or "error" in file_name.lower():  # 오류 발생 시 체크
            return jsonify({"status": "error", "message": "크롤링 중 오류 발생"}), 500
        
        app.config['FILE_NAME'] = file_name

        # 작업 완료 후 jSON 응답 반환
        return jsonify({"status": "running", "message": "Selenium done"})
